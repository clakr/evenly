...
export function SummaryTabContent({ items, findParticipantName }: Props) {
		...
		itemsBreakdown.forEach((item) => {
			item.distributions.forEach((distribution) => {
				...
				const payload = createEmptyParticipantItems({
					...
					name: findParticipantName(distribution.participantId),
					...participant,
				});
	...
	}, [itemsBreakdown, findParticipantName]);
